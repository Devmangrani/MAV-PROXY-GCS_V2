...
class GPSSITLFollow:
    ...
    def follow_gps(self):
        ...
        logger.info(f"Starting GPS follow with {self.distance_threshold}m distance threshold and Kalman filtering")
                        ...
                        if msg.sentence_type == 'GGA':
                            if msg.latitude and msg.longitude:
                                ...
                                if not filter_initialized:
                                    ...
                                    # Calculate current speed if we have previous positions
                                ...
                                logger.info(
                                    f"Distance to target: {distance:.2f}m (threshold: {self.distance_threshold}m)\n"
                                    f"Raw GPS: {raw_lat:.6f}, {raw_lon:.6f}\n"
                                    f"Filtered: {filtered_lat:.6f}, {filtered_lon:.6f}\n"
                                    f"Smoothed: {smoothed_lat:.6f}, {smoothed_lon:.6f}\n"
                                    f"Estimated Speed: {current_speed:.2f} m/s"
                                )

                                # Only send command if distance exceeds threshold
                                if distance > self.distance_threshold:
                                    ...
                                    logger.info(
                                        f"Position command sent (distance: {distance:.2f}m):\n"
                                    ...
                                    logger.info(
                                        f"Skipping command - distance {distance:.2f}m below threshold {self.distance_threshold}m")
...
def main():
    try:
        parser = argparse.ArgumentParser(description='GPS Follow with distance threshold')
