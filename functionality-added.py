...
def fetch_data():
    global vehicle
    message_types = ["ATTITUDE", "GLOBAL_POSITION_INT", "VFR_HUD", "SYS_STATUS", "STATUSTEXT", "BATTERY_STATUS"]
    ...
    while True:
        ...
        try:
            msg = vehicle.recv_match(type=message_types, blocking=True, timeout=5)
            if msg:
                ...
                logger.warning("No message received in the last 5 seconds")
                socketio.emit('message', {'text': "No message received in the last 5 seconds", 'color': 'orange'})
        except Exception as e:
...
@app.route('/')
def index():
    return render_template('functionality-added.html')
...
def get_mode_id(mode_name):
    mode_map = {
        ...
        'ZIGZAG': 24, 'SYSTEMID': 25, 'AUTOROTATE': 26
    }
    return mode_map.get(mode_name.upper(), 0)
