...
def handle_takeoff(altitude):
    ...
    try:
        ...
        if not ack or ack.command != mavutil.mavlink.MAV_CMD_NAV_TAKEOFF or ack.result != mavutil.mavlink.MAV_RESULT_ACCEPTED:
            socketio.emit('message', {'text': f"Takeoff command failed. Result: {ack.result if ack else 'No acknowledgement'}", 'color': 'red'})
            return
        ...
        while time.time() - start_time < 30:  # Wait up to 30 seconds
            ...
            if msg:
                ...
                if current_altitude >= altitude * 0.95:  # Within 5% of target altitude
                    socketio.emit('message', {'text': f"Reached target altitude of {altitude}m", 'color': 'green'})
                    return
            time.sleep(1)
...
@app.route('/')
def index():
    return render_template('functionality_added_v6.html')
...
#     wp_loader.insert(0, wp)
#
#     return jsonify({'message': f'Takeoff added: Lat {lat}, Lon {lon}, Alt {alt}m'})
#
#
...
#     wp_loader.add(wp)
#
#     return jsonify({'message': f'Landing added: Lat {lat}, Lon {lon}'})
#
#
# @app.route('/add_rtl')
# def add_rtl():
...
#     wp_loader.add(wp)
#
#     return jsonify({'message': 'RTL added to mission'})
...
def execute_mission():
    with vehicle_lock:
        ...
        try:
            data = request.json
            takeoff_altitude = data.get('takeoff_altitude', 10)  # Default to 10m if not provided

            # Check if the vehicle is already in the air
            if is_vehicle_in_air():
                logger.info("Vehicle is already in the air. Proceeding with mission execution.")
            else:
                # Arm the vehicle
                vehicle.mav.command_long_send(
                    vehicle.target_system, vehicle.target_component,
                ...
                for _ in range(30):  # Wait up to 30 seconds
                    ...
                    return jsonify({'error': 'Takeoff failed'}), 400

            # Switch to AUTO mode to start the mission
            result = set_mode('AUTO')
            if result['success']:
                return jsonify({'message': 'Takeoff completed and mission started in AUTO mode'})
            else:
    ...
    with vehicle_lock:
        ...
        try:
            ...
            if ack and ack.command == mavutil.mavlink.MAV_CMD_DO_SET_MODE and ack.result == mavutil.mavlink.MAV_RESULT_ACCEPTED:
                return jsonify({'message': 'Successfully switched to AUTO mode'})
            else:
